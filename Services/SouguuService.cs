...
public class SouguuService : ISouguuService {
  ...
  public async Task CancelCall(int userIndex,
                               CancelCallWebsocketDto cancelCallWebsocketDto) {
    var requestIds = _requestIdToUserIndex.Where(i => i.Value == userIndex)
                         .Select(i => i.Key)
                         .ToList();
    var actions = _callCancelNotification.Where(i => requestIds.Contains(i.Key))
                      .Select(i => i.Value)
                      .ToList();

    actions.ForEach(i => i(userIndex, cancelCallWebsocketDto));
  }
