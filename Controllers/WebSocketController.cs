    ...
    : ControllerBase
{
                                 ...
                                 CancellationToken cancellationToken)
    {
        while (webSocket.State == WebSocketState.Open)
        {
            ...
            {
                await webSocket.SendAsync(Encoding.UTF8.GetBytes("battari"),
    ...
    private void SendNotification(WebSocket websocket, int userId,
                                  WebsocketDtoForSend dto, string requestKey)
    {
        if (userOnlineConcurrentDictionaryDatabase.IsUserOnline(userId))
        {
            if (websocket.State == WebSocketState.Open)
            {
                try
                {
                    var json = JsonSerializer.Serialize<WebsocketDtoForSend>(
                    ...
                    SentrySdk.CaptureMessage("Websocket Send notification:" + json +
                                                 userId + "requestKey:" + requestKey,
                                             SentryLevel.Info);
                }
                catch (Exception e)
                {
        ...
        else
        {
            SentrySdk.CaptureMessage(
                "Websocket Send notification: user is not online " + userId,
                SentryLevel.Warning);
        }
    }
    ...
    public async Task Get()
    {
        ...
        bool isEnd = false;
        int userId = Int16.Parse((HttpContext.User.Claims.FirstOrDefault(
            c => c.Type ==
                 "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/name"))!
                                     .Value);

        // #TODO
        // ワンチャンbufferを超えたデータを受け取った場合，受け取れきれないかも
        ...
        if (HttpContext.WebSockets.IsWebSocketRequest)
        {
            ...
            while (webSocket.State == WebSocketState.Open)
            {
                ...
                try
                {
                    ...
                    do
                    {
                        ...
                        if (result.CloseStatus.HasValue)
                        {
                            ...
                            SentrySdk.CaptureMessage($"{userId}:WebSocket接続終了",
                                                     SentryLevel.Info);
                            isEnd = true;
                            break;
                        }
                ...
                if (buffer.Length > 0)
                {
                    logger.LogInformation("Received: {received} from {userId}", received,
                                          userId);
                    if (received == "hello")
                    {
                    }
                    else
                    {
                        try
                        {
                            ...
                            WebsocketDto? parsed =
                                JsonSerializer.Deserialize<WebsocketDto>(received, _options);
                            if (parsed == null)
                                continue;
                            if (parsed.type.Equals("souguu_materials"))
                            {
                                SouguuWebsocketDto? souguuWebsocketDto =
                                    JsonSerializer.Deserialize<SouguuWebsocketDto>(
                                        parsed.data.ToString());
                                if (souguuWebsocketDto == null)
                                    continue;
                                if (souguuWebsocketDto
                                        .incredients[0] is SouguuAppIncredientModel)
                                {
                                    SouguuAppIncredientModel app =
